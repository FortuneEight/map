var Graphics = {
    ...
    draw: function() {
        ...
        var horizon = 0.3;
        // var bg = context.createLinearGradient(255, 0, 0, this.canvas.height);
        var bg = context.createLinearGradient(255, 0, 0, this.canvas.height);
        bg.addColorStop(horizon, "hsl(150,0%,0%)");
        bg.addColorStop(0, "hsl(150,0%," + (Math.pow(.15, gamma) * 100) + "%)");
        bg.addColorStop(1, "hsl(150,0%," + (Math.pow(.15, gamma) * 100) + "%)");
        ...
        if (Settings.graphics.shadows) {
            for (var i = 2; i < scene.length; i += 8) {
                ...
                if (vLeftEd || true && vLeftD < shadScale) {
                    var opacity = 1 - Math.pow(vLeftX, 12);
                    opacity *= 0.2;
                ...
                if (vRightEd || true && vRightD < shadScale) {
                    var opacity = 1 - Math.pow(vRightX - 1, 12);
                    opacity *= 0.2;
                    context.fillStyle = "rgba(0,0,0," + opacity + ")";
                    context.beginPath();
    ...
    drawWall: function(canvas, context, scene, horizon, i, gamma, petalEffect) {
        ...
        if (Settings.graphics.gradients) {
            ...
            // g.addColorStop(0, "rgba(35, 100, 231, 0.5)");
            // g.addColorStop(1, "rgba(35, 100, 231, 0.5)");
            g.addColorStop(0, "hsl(150,0%," + (Math.floor(Math.pow(leftColor, gamma) * 1000) / 10) + "%)");
            g.addColorStop(1, "hsl(150,0%," + (Math.floor(Math.pow(rightColor, gamma) * 1000) / 10) + "%)");
            context.fillStyle = g;
        } else {
